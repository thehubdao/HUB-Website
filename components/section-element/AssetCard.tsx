    ...
    return (
        ...
        <a href={link} target="_blank">
            <div className={`${classes} text-center hover:scale-105 hover:z-50 group shadow-colorbottom hover:opacity-100 relative scale-100 flex flex-col w-full rounded-2xl bg-gray-400 bg-opacity-10 transform transition duration-300 ease-in-out hover:shadow-color border border-white border-opacity-10 hover:border-opacity-50 cursor-pointer`}>
                ...
                {metaverse === "Decentraland" && <img src="/images/Logos/Cryptos/webp/MANA.webp" className="absolute top-2 right-2 h-8" />}
                {metaverse === "Sandbox" && <img src="/images/Logos/Cryptos/webp/SAND.webp" className="absolute top-2 right-2 h-8" />}
                {metaverse === "ASM" && <img src="/images/Logos/webp/asm-logo.webp" className="absolute top-2 right-2 h-8" />}
                {metaverse === "PartyBear" && <img src="/images/Logos/webp/partybear-logo.webp" className="absolute top-2 right-2 h-8" />}
                {metaverse === "FLUFWorld" && <img src="/images/Logos/webp/flufworld-logo.webp" className="absolute top-2 right-2 h-8" />}
